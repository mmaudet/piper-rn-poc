...
- The cold-start TTFA includes a 700-800 ms model load on the first 4 voices; the next run on the same language will be ~750 ms faster.
- **`es_ES-sharvard-medium` is the outlier** — the archive is 77 MB vs ~64 MB for the others, so the cold model load is 1.6 s vs 0.7-0.8 s, and the per-sentence VITS inference is ~2× slower on this voice. Still well within spec, just slower than its siblings.
- The TTFA-to-inference relationship is mostly: `TTFA ≈ modelLoad + ~1.6s native synth (first sentence) + ~1s of (WAV write + Sound load + RN bridge)`. The RN bridge / `react-native-sound` warmup accounts for ~1 s that we can chip away at with a better audio path in a v2.
- The inference RTF stays remarkably consistent across languages (4.46× → 6.62×). The eSpeak-NG phonemizer + Piper VITS path is well-behaved on Tensor G5 CPU without any acceleration tuning.
